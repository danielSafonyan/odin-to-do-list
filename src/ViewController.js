...
class ViewController {
  ...
  calculateRemainingTasks() {
    ...
    if (this.currentProjectID === 'all-tasks') {
      ...
      numRemainingTasks = allTasks.filter(
        (task) => task.isDone === false,
      ).length;
    } else {
      numRemainingTasks = Object.values(
        data.cache[this.currentProjectID].taskList,
      ).filter((task) => task.isDone === false).length;
    }
...
function projectClickEvent(event) {
  ...
  const currentProjectDisplayed = document.querySelector(
    `#${currentProject.currentProjectID}`,
  );
  currentProjectDisplayed.classList.remove('active-project');
  const newProjectDisplayed = document.querySelector(
